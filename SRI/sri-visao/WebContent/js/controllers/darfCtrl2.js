...
app.controller('DARFCtrl2', ['$scope','$routeParams', '$rootScope', '$location','DARFSrvc2', 'AlertApp', 'TIPO_ALERTAS', 'RetencaoSrvc', 'UFSrvc', 'MunicipioSrvc','TipoSrvc','RiscoSrvc',
function($scope, $routeParams,  $rootScope, $location, service, alertApp, TIPO_ALERTAS, retencaoService, ufService, municipioService, tipoService, riscoService) {
	
	// controller para tetes de acesso ao WSO2 com cabeçalho Authorization contendo o Acess Token
	...
	function carregarRetencaoList(){
    	//Carregar orgao
		retencaoService.findAllAccessToken().then(
			function (data) {			
				console.log(data);
    ...
    function carregarUFList(){
    	//Carregar UFs
		ufService.findAllAccessToken().then(
			function (data) {			
				console.log(data);
				$scope.ufList = data;
			},
    ...
    function carregarMunicipioList(){
    	//Carregar municipios
		municipioService.findAllAccessToken().then(
			function (data) {			
				console.log(data);
    ...
    function carregarTipoList(){
    	//Carregar tipos
		tipoService.findAllAccessToken().then(
			function (data) {			
				console.log(data);
				$scope.tipoList = data;
			},
    ...
    function carregarRiscoList(){
    	//Carregar riscos
		riscoService.findAllAccessToken().then(
			function (data) {			
				console.log(data);
			...
			service.save($scope.darf).then(
				function (data) {
					...
					if ($scope.darf.id != undefined){
						...
						$location.path("/darf/edit2/" + $scope.darf.id);    	
					}
					else{ // POST
						$location.path('/darf/list2');
    ...
    $scope.gridOptions2 = {
            ...
            columnDefs: [{field: 'nrDARF', displayName: 'Número', width: "14%"},
                ...
                {displayName: 'Ação', cellTemplate: '<a ng-show="!currentUser" ng-click="edit(row.entity.id)" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-eye-open"></i>Visualizar</a>\n\
                                                 <a ng-show="currentUser" ng-click="edit(row.entity.id)" class="btn btn-warning btn-sm"><i class="glyphicon glyphicon-plus-sign"></i>Alterar</a>\n\
                                                 <a confirm-button title="Excluir?" confirm-action="delete(row.entity.id,row.entity.nrDARF)" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-minus-sign"></i>Excluir</a>', width: "22%"}],
            selectedItems: [],
